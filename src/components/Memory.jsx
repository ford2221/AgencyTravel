...
const Memory = ({memories: {title, text, subtitle, img, experience} }) => {
  return (
    <>
      <div className='relative my-16 md:mb-7'>
        <div className='travigo-container flex items-center justify-between gap-16 xl:gap-9 px-10
          lg:flex-col-reverse'>
          <div className='flex items-center justify-start lg:justify-center w-full max-w-md'>
            <img src={img} alt="" className='w-auto h-[55vh] object-fill'/>
          </div>
          <div className='grid items-center w-full max-w-2xl lg:text-center'>
            ...
            <div className='flex flex-wrap gap-7 px-10'>
              {experience?.map((val, i) => (
                <div key={i} className="bg-gradient-to-b from-emerald-500 to-teal-300 shadow-lg shadow-emerald-200 flex
                  items-center justify-center flex-col text-slate-900 py-7 px-5 xl:p-5 rounded-lg filter
